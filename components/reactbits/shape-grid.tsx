...
const ShapeGrid: React.FC<ShapeGridProps> = ({
  ...
  useEffect(() => {
    ...
    const drawGrid = () => {
      ...
      // transparent stop must share the vignette's RGB, otherwise the
      // gradient interpolates through semi-transparent gray
      const transparentStop = vignetteColor.startsWith('#')
        ? `rgba(${parseInt(vignetteColor.slice(1, 3), 16)}, ${parseInt(
            vignetteColor.slice(3, 5),
            16
          )}, ${parseInt(vignetteColor.slice(5, 7), 16)}, 0)`
        : 'rgba(0, 0, 0, 0)';
      gradient.addColorStop(0, transparentStop);
    ...
    // listen on window so the hover effect works beneath overlaid hero content
